...
/* Empty IC */
void blank_ic() { SDL_RenderCopy(renderer, base_texture, NULL, NULL); }
...
void update_speed() {
    ...
    if (angle > 180)
        angle = 180;
    SDL_RenderCopyEx(renderer, needle_tex, NULL, &speed_rect, angle, &center,
                     SDL_FLIP_NONE);
}
...
int main(int argc, char *argv[]) {
    ...
    if (stat(DATA_DIR, &dirstat) == -1) {
        printf("ERROR: DATA_DIR not found.  Define in make file or run in src "
               "dir\n");
        exit(34);
    }
    ...
    window =
        SDL_CreateWindow("IC Simulator", SDL_WINDOWPOS_UNDEFINED,

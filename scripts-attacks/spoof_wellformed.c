/*
 * spoof_wellformed.c — spoofing BEM-FORMADO para o gateway SecOC-FD.
 * 
 * Emite frames CAN FD com ID associado válido, comprimento secured correto
 * (plain_len + SECOC_OVERHEAD) e FV DENTRO da janela de freshness, mas com MAC
 * inválido. O frame passa ID -> LEN -> FV e bate no MAC, onde falha (o atacante
 * não tem a chave).
 */
...
/* Perfil FD do SecOC-Lite */
#define DEF_OVERHEAD 12   /* SECOC_FV_LEN(4) + SECOC_MAC_LEN(8) */
...
static int open_fd_socket(const char *iface)
{
    int s = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    if (s < 0) { perror("socket(PF_CAN)"); return -1; }
 
    int en = 1;
    if (setsockopt(s, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &en, sizeof(en)) < 0) {
        perror("setsockopt(CAN_RAW_FD_FRAMES)"); close(s); return -1;
    }
    struct ifreq ifr;
...
static void usage(const char *p)
{
    fprintf(stderr,
        ...
        "      --plain-len <n>  = expected_plain_len do build do gateway\n"
        "      --overhead <n>   SECOC_OVERHEAD (default %d)\n"
        "      --fv <n>         FV em claro, dentro da janela (default 0)\n"
...
int main(int argc, char **argv)
{
    ...
    if (secured_len < 0 || secured_len > 64) {
        fprintf(stderr, "secured_len=%d fora de 0..64 (FD). plain=%d overhead=%d\n",
    ...
    memset(&f, 0, sizeof(f));
    f.can_id = id;
    f.len    = (uint8_t)secured_len;
    f.flags  = 0;
    uint32_t rng = 0xC0FFEEu ^ (uint32_t)id ^ (uint32_t)plain_len;
    fill_garbage(f.data, secured_len, &rng);   /* payload + FV + MAC lixo */
    put_be32(&f.data[plain_len], fv);          /* sobrescreve FV com valor valido */
    /* MAC (ultimos 8B) fica lixo -> falha na verificacao. */

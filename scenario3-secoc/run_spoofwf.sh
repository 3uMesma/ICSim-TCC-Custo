#!/usr/bin/env bash
# Campanha minima do spoof bem-formado.
...
CSV="$PWD/results/perf_spoofwf.csv"
REPS="${REPS:-20}"
GW="$PWD/secoc_gateway_p4"      # espera plain=5 

mkdir -p "$PWD/results"
echo "[campanha] gateway=$GW  reps=$REPS"
echo "[campanha] csv=$CSV"

for r in $(seq 1 "$REPS"); do
    echo "===== rep $r / $REPS ====="
    COMPONENT=hit  SPOOF_ID=0x244 SPOOF_PLAIN_LEN=5 \
        GATEWAY_BIN="$GW" RUN_INDEX="$r" PERF_DATA_CSV="$CSV" \
        ./run_scenario3.sh spoof-wf 30
    COMPONENT=miss SPOOF_ID=0x555 SPOOF_PLAIN_LEN=5 \
        GATEWAY_BIN="$GW" RUN_INDEX="$r" PERF_DATA_CSV="$CSV" \
        ./run_scenario3.sh spoof-wf 30
done

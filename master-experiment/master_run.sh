...
ensure_replay_capture() {
    if echo "$ATTACKS_BASELINE" | grep -qw replay; then
        if [[ ! -s "$REPLAY_LOG" ]]; then
            ...
            python3 "$ATAQUES_DIR/03_replay_attack.py" record \
                --iface vcan0 \
                --out   "$REPLAY_LOG" \
...
postprocess() {
    if [[ -x "$HERE/analyze_all.py" && -s "$PERF_DATA_CSV" ]]; then
        log ""
        log "Rodando analyze_all.py sobre $PERF_DATA_CSV…"
    ...
    log "  summary_all.csv           — estatísticas por cenário/ataque"
    log "  comparison_overhead.csv   — overhead %% vs baseline (wide)"
    log "  comparison_overhead.tex   — tabela LaTeX para o TCC"
    log "  cenario2/runs/            — logs auxiliares (gateway.log, attack.log)"
    log "  cenario3/runs/            — logs auxiliares (gateway, sender, attack)"
}

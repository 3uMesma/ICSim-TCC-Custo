...
void send_turn_signal() {
    ...
    if (signal_pos)
        randomize_pkt(0, signal_pos);
    if (signal_len != signal_pos + 1)
        randomize_pkt(signal_pos + 1, signal_len);
    send_pkt(CAN_MTU);
}
...
void play_can_traffic() {
    ...
    if (execlp("canplayer", "canplayer", "-I", traffic_log, "-l", "i", can2can,
               NULL) == -1)
        printf("WARNING: Could not execute canplayer. No bg data\n");
}

void kill_child() { kill(play_id, SIGINT); }
...
void print_joy_info() {
    ...
    if (strncmp(SDL_JoystickNameForIndex(0), "PLAYSTATION(R)3 Controller",
                25) == 0) {
        // PS3 Rumble controller via BT
        gControllerType = PS3_CONTROLLER;
    }
    if (strncmp(SDL_JoystickNameForIndex(0), "Sony PLAYSTATION(R)3 Controller",
                30) == 0) {
...
void usage(char *msg) {
    ...
    printf("\t-X\tDisable background CAN traffic.  Cheating if doing RE but "
           "needed if playing on a real CANbus\n");
    printf("\t-d\tdebug mode\n");
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    window =
        SDL_CreateWindow("CANBus Control Panel", SDL_WINDOWPOS_UNDEFINED,
        ...
        while (SDL_PollEvent(&event) != 0) {
            ...
            case SDL_KEYDOWN:
                ...
                case SDLK_LSHIFT:
                    lock_enabled = 1;
                    if (unlock_enabled)
                        send_lock(CAN_DOOR1_LOCK | CAN_DOOR2_LOCK |
                                  CAN_DOOR3_LOCK | CAN_DOOR4_LOCK);
                    break;
                case SDLK_RSHIFT:
                    unlock_enabled = 1;
                    if (lock_enabled)
                        send_unlock(CAN_DOOR1_LOCK | CAN_DOOR2_LOCK |
                                    CAN_DOOR3_LOCK | CAN_DOOR4_LOCK);
                    break;
                case SDLK_a:
                    if (lock_enabled) {
            ...
            case SDL_JOYAXISMOTION:
                ...
                } else if (axis == gAxisRightH || axis == gAxisRightV ||
                           axis == gAxisL2 || axis == gJoyX || axis == gJoyY ||
                           axis == gJoyZ) {
